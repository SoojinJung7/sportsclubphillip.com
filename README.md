...
src/
  ...
  pages/                ← 각 페이지 (index, about, operating, projects, promotion,
                          pilates, career, schedule, swimminglesson, academy, kpa, bally,
...
| 메뉴 | 내용 |
|---|---|
| **📅 이달의 포스터** | **매달 바뀌는 포스터 8종을 한 화면에서 업로드** — 영업안내 · GX · 수영(스케줄/안내) · 프로모션 1~4. 여기 올린 파일이 사진 목록보다 우선합니다. 비우면 기존 포스터 유지 |
| **사이트 설정** | 전화·팩스·이메일·주소·대표이사·사업자번호, 상담시간, SNS/예약 링크 |
| **사진** | 로고·시설 사진 등 **나머지 모든 사진**을 파일 업로드로 교체 + 대체 텍스트 (포스터의 대체 텍스트도 여기서 수정) |
...
| **💬 상담 버튼 마스코트** | 시즌 한정 캐릭터 영상으로 오른쪽 아래 '상담 문의' 버튼을 대체 (추석 토끼). 알약 문구가 영상에 박혀 있어 한국어('상담 문의')·영어('Chat') 두 벌이 있고 언어 토글에 맞춰 자동으로 바뀝니다. 끄면 평소의 오렌지 버튼으로 돌아갑니다 |

> **매달 포스터 교체**: **📅 이달의 포스터** 메뉴(왼쪽 맨 위) 하나만 열어서 새 포스터를 올리면
> 홈 팝업·This Month 캐러셀·시간표·수영강습·프로모션 페이지에 한 번에 반영됩니다.
>
> **그 밖의 사진 교체**: CMS의 **사진** 메뉴에서 원하는 항목의 "사진 교체"에 파일을 업로드하면 끝입니다
...
1. **이달의 포스터** — `content/posters.json` 에 그 키가 채워져 있으면(= 📅 이달의 포스터 메뉴 업로드) 최우선입니다.
2. **CMS 업로드** — `content/media.json` 의 `src` 가 채워져 있으면(=`public/uploads/…` 업로드) 그 사진을 씁니다.
3. **내장 기본** — 비어 있으면 `src/assets/images/` 의 최적화 원본을 빌드 시
